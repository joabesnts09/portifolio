...
export const useScroll = () => {
    if (typeof window !== "undefined") {
        window.addEventListener('scroll', () => {
            ...
            const secAbout = document.querySelector('#aboutme') as HTMLAnchorElement
            const secSkill = document.querySelector('#skills') as HTMLAnchorElement
            const secProjects = document.querySelector('#projects') as HTMLAnchorElement
            ...
            const sectionProjects = document.querySelector('#project') as HTMLAnchorElement
    
            header.classList.toggle(styles.scroll, window.scrollY > 0)
            arrouUp.classList.toggle(style.scrollArrow, window.scrollY > 600)
            
            if(window.scrollY > 100 && window.scrollY < secAbout.scrollHeight) { 
                sectionAbout.classList.add(styles.actived)
                sectionSkills.classList.remove(styles.actived)
            } else if (window.scrollY >= secAbout.scrollHeight  && window.scrollY < secAbout.scrollHeight + secSkill.scrollHeight) {
                // console.log(secSkill.scrollHeight);
            ...
            }  else if (window.scrollY >= secAbout.scrollHeight + secSkill.scrollHeight) {
                console.log(secProjects.scrollHeight);
